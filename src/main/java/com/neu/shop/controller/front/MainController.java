...
public class MainController {
    ...
    public String showAllGoods(Model model, HttpSession session) {
        ...
        //数码分类
        List<Goods> digGoods = getCateGoods("川菜", userid);
        model.addAttribute("digGoods", digGoods);

        //家电
        List<Goods> houseGoods = getCateGoods("东北菜", userid);
        model.addAttribute("houseGoods", houseGoods);

        //服饰
        List<Goods> colGoods = getCateGoods("粤菜", userid);
        model.addAttribute("colGoods", colGoods);

        //书籍
        List<Goods> bookGoods = getCateGoods("湘菜", userid);
        model.addAttribute("bookGoods", bookGoods);

        List<Goods> xiangCai = getCateGoods("湘菜", userid);
        model.addAttribute("xiangCai", xiangCai);
